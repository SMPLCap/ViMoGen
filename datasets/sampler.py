...
from utils import format_numel_str
...
class VariableVideoBatchSampler(DistributedSampler):
    ...
    def __iter__(self) -> Iterator[List[int]]:
        ...
        if self.shuffle:
            ...
            bucket_id_access_order = [
                bucket_id_access_order[i]
                for i in bucket_id_access_order_indices
            ]
        ...
        if remainder > 0:
            if self.drop_last:
                bucket_id_access_order = bucket_id_access_order[:len(
                    bucket_id_access_order) - remainder]
            else:
                bucket_id_access_order += bucket_id_access_order[:self.
                                                                 num_replicas -
                                                                 remainder]
        ...
        for i in range(start_iter_idx, num_iters):
            bucket_access_list = bucket_id_access_order[i *
                                                        self.num_replicas:(i +
                                                                           1) *
                                                        self.num_replicas]
            ...
            cur_micro_batch = bucket_sample_dict[bucket_id][
    ...
    def _print_bucket_info(self, bucket_sample_dict: dict) -> None:
        ...
        # sort
        num_aspect_dict = dict(
            sorted(num_aspect_dict.items(), key=lambda x: x[0]))
        num_hwt_dict = dict(
            sorted(
                num_hwt_dict.items(),
                key=lambda x: (get_num_pixels(x[0][0]), x[0][1]),
                reverse=True,
            ))
        ...
        if dist.get_rank() == 0 and self.verbose:
            ...
            get_logger().info(
                ...
                pformat(num_aspect_dict, sort_dicts=False),
            )
            get_logger().info(
                'Image Bucket [#sample, #batch] by HxWxT:\n%s',
                pformat(num_hwt_img_dict, sort_dicts=False),
            )
            get_logger().info(
                'Video Bucket [#sample, #batch] by HxWxT:\n%s',
                pformat(num_hwt_vid_dict, sort_dicts=False),
            )
            get_logger().info(
...
class BatchDistributedSampler(DistributedSampler):
    """Used with BatchDataset; Suppose len_buffer == 5, num_buffers == 6,
    ...
    def __iter__(self):
        ...
        indices_i = (
            np.arange(self.start_index, num_samples_i) +
            self.rank * num_samples_i)
