...
# keep exported parameters (inference.py saves them into OUTPUT_PATH)
PARAMS_PATH="${OUTPUT_PATH}/${NAME}_params.pt"
if [ -f "$PARAMS_PATH" ]; then
    cp "$PARAMS_PATH" "./demo/${NAME}_params.pt"
    echo "Saved SMPL-X parameters to ./demo/${NAME}_params.pt"
fi
...
case "$EXT" in
    mp4|avi|mov|mkv|flv|wmv|webm|mpeg|mpg)
        ffmpeg -y -f image2 -r ${FPS} -i ${OUTPUT_PATH}/%06d.jpg -c:v libopenh264 -pix_fmt yuv420p ./demo/result_${NAME}.mp4
        ;;
    jpg|jpeg|png|bmp|gif|tiff|tif|webp|svg)
