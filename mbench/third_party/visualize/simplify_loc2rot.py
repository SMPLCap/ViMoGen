...
import os
import torch
from mbench.third_party.visualize.joints2smpl.src import config
import smplx
import h5py
from mbench.third_party.visualize.joints2smpl.src.smplify import SMPLify3D
from tqdm import tqdm
import mbench.third_party.visualize.rotation_conversions as geometry
import argparse
...
class joints2smpl:
    ...
    def joint2smpl(self, input_joints, init_params=None):
        ...
        root_loc = keypoints_3d[:, 0].clone().detach()
        ...
        return new_opt_pose.clone().detach(), thetas.clone().detach(), {'pose': new_opt_joints[0, :24].flatten().clone().detach(), 'betas': new_opt_betas.clone().detach(), 'cam': new_opt_cam_t.clone().detach()}
