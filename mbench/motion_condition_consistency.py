...
def compute_motion_condition_consistency(full_info_path: str, device: str,
    ...
    prompt_dict_ls = load_dimension_info(full_info_path, dimension='Motion_Condition_Consistency')
    ...
    Then, based on this detailed motion analysis, select the closest match from the provided motion list. Output the most relevant motion.
    Example output is: "The person in the video is standing and clapping their hands. The closest match from the provided motion list is: clapping" 
    The provided motion list is """
    ...
    def evaluate_entry(prompt_dict):
        ...
        motion_list = auxiliary_info.copy()
        if video_label not in motion_list:
            motion_list.append(video_label)
        random.shuffle(motion_list)
        result_string = ", ".join(motion_list)

        try:
            response = call_gemini_api(video_bytes, prompt + f'"{result_string}"', api_key, fps=vlm_fps)
        ...
        response_clean = response_clean.split(":", 1)[-1].strip()
        # Remove trailing punctuation (., ", ', etc.) that may cause false negatives
        response_clean = response_clean.rstrip('.,;:!?\'"')

        is_correct = response_clean.lower() == video_label.lower()
        return {
            "skip": False,
            "id": motion_id,
    ...
    with ThreadPoolExecutor(max_workers=gemini_workers) as executor:
        ...
        for future in tqdm(as_completed(futures), total=len(futures), desc="Gemini MCC"):
            try:
            ...
            per_motion_metrics.append(
                {
                    ...
                    "prediction": result["response_clean"],
                    "video_path": result["video_path"],
                    "candidates": result["motion_list"],
